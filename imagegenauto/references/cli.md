...
For this customized skill, start the CLI without an extra confirmation. On Windows use `scripts/invoke_image_gen_auto.ps1`, which detects an existing Python runtime or downloads and silently installs Python 3.14.6 from the configured Aliyun mirror, verifies the user `Path` contains Python and its `Scripts` directory, then ensures `openai` is installed. Codex may not inherit `Path` updates until restart, so the launcher directly runs the installed `python.exe` for the current job. It calls `scripts/image_gen_auto.py`, which retries a job three times on `gpt-image-2`, then repeats it with `gpt-image-1.5` and `gpt-image-1` only for transient service failures. Do not fall back when the user explicitly selected a model, the failure is a prompt/policy/parameter error, or the API key is missing. Verify a non-empty output file before declaring success.
