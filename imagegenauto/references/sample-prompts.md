...
These prompt recipes are for the bundled `scripts/image_gen.mjs` Node CLI.
...
Execution details such as `quality`, `input_fidelity`, masks, output formats, and local output paths are CLI controls. For transparent images, prompt for a flat chroma-key background and remove it locally with `scripts/remove_chroma_key.mjs`, or use `gpt-image-1.5` native transparency for complex edges.

CLI model notes:
- `gpt-image-2` is the CLI default for new workflows unless configured through `OPENAI_IMAGE_MODEL`.
- `gpt-image-2` supports `quality` values `low`, `medium`, `high`, and `auto`.
- For 4K-style `gpt-image-2` output, use `3840x2160` or `2160x3840`.
- Use `gpt-image-1.5` when true native transparency is required because `gpt-image-2` does not support `background=transparent`.
...
Post-process note: run `%USERPROFILE%\.codex\skills\noderuntime\scripts\run-node.cmd "%USERPROFILE%\.codex\skills\imagegenauto\scripts\remove_chroma_key.mjs" --input <source> --out <final.png> --auto-key border --soft-matte --transparent-threshold 12 --opaque-threshold 220 --despill`. Use `gpt-image-1.5 --background transparent --output-format png` for true native transparency or complex subjects such as hair, fur, glass, smoke, liquids, translucent materials, reflections, or soft shadows.
