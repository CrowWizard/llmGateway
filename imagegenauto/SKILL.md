---
name: "imagegenauto"
description: "Generate or edit raster images with a completion-first OpenAI Image CLI workflow. Use when Codex should create a new bitmap image, edit an existing bitmap image, or generate multiple variants and should automatically retry transient API failures and fall back across GPT Image models without pausing for routine confirmation."
---
...
This customized skill uses the bundled CLI as its default execution path. It requires `OPENAI_API_KEY`. On Windows, begin through `scripts/invoke_image_gen_auto.ps1`: it uses an existing Python installation or downloads and silently installs Python 3.14.6 from the configured Aliyun mirror, verifies Python and `Scripts` are in the user `Path`, and installs `openai` for the current user. Because Codex can retain its pre-install environment block, the launcher uses the installed directory's absolute `python.exe` immediately rather than waiting for `Path` to refresh.
...
Rules:
- Start the CLI immediately for ordinary image requests; do not ask a redundant confirmation when `OPENAI_API_KEY` is already configured.
- Treat this skill as authorization to recover from routine transient service failures without asking again.
- Run `scripts/invoke_image_gen_auto.ps1` on Windows; it ensures Python and `openai` are available, then runs `image_gen_auto.py`. The executor retries `gpt-image-2` three times and, for transient availability, timeout, or server errors, retries the same request with `gpt-image-1.5`, then `gpt-image-1`.
- Preserve the user's selected model when one is explicitly named; retry that model three times first, then report its failure instead of silently changing an explicitly selected model.
- For a true transparent-background request, use `gpt-image-1.5 --background transparent --output-format png` directly.
- Save completed deliverables under the current project's `outputs/` directory unless the user gives a destination.
- Report only the final artifact path and the model that succeeded. Report intermediate retries only if every attempt fails.
- Do not invent a success: verify that the expected output file exists and is non-empty before reporting it.
...
- If the CLI cannot run because `OPENAI_API_KEY` is missing, say so once and provide the local setup instruction. Do not ask for the key in chat.
...
## Workflow
1. Default to CLI generation and begin immediately when `OPENAI_API_KEY` is available.
...
10. On Windows, run `scripts/invoke_image_gen_auto.ps1`; on other systems, run `scripts/image_gen_auto.py` with a stable descriptive output filename, and use `--force` only when the user requests replacement.
...
17. If the initial model exhausts retries due to a transient service failure, automatically use the fallback model sequence defined above; keep the prompt, dimensions, and quality unchanged where supported.
