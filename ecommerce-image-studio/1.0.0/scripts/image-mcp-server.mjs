...
const tools = [
  ...
  {
    name: "image_generate",
    description: "Generate an ecommerce product image. Returns the generated image directly to Codex. Automatically retries with gpt-image-1.5 only when gpt-image-2 is explicitly unavailable.",
    inputSchema: {
      type: "object",
  ...
  {
    name: "image_edit",
    description: "Edit a local product image using an instruction. Optional mask marks editable pixels. Automatically retries with gpt-image-1.5 only when gpt-image-2 is explicitly unavailable.",
    inputSchema: {
      type: "object",
      additionalProperties: false,
...
class ProviderRequestError extends Error {
  constructor(status, detail, statusText) {
    super(`Image provider returned ${status}: ${detail || statusText}.`);
    this.status = status;
    this.detail = detail;
  }
}
...
async function providerRequest(provider, path, options) {
  ...
  if (!request.ok) {
    ...
    throw new ProviderRequestError(request.status, detail, request.statusText);
  }
...
function modelUnavailable(error) {
  return error instanceof ProviderRequestError
    && /\bmodel_not_found\b|no available channel for model|model (?:is )?not found/i.test(error.detail);
}

async function requestWithFallback(provider, args, request) {
  const initialModel = selectedModel(args, provider);
  try {
    return { image: await request(initialModel), model: initialModel, fallbackUsed: false };
  } catch (error) {
    if (initialModel !== "gpt-image-2" || !modelUnavailable(error)) throw error;
    return { image: await request(FALLBACK_MODEL), model: FALLBACK_MODEL, fallbackUsed: true };
  }
}

async function imageResult(image, outputPath, fallbackUsed) {
  const content = [{ type: "image", data: image.data, mimeType: "image/png" }];
  if (fallbackUsed) content.unshift({ type: "text", text: `Automatic model fallback: gpt-image-2 was unavailable, so this image was generated with ${FALLBACK_MODEL}.` });
...
function generationBody(args, model) {
  return JSON.stringify({ model, prompt: args.prompt, size: args.size ?? "auto", quality: args.quality ?? "auto", background: args.background ?? "auto", response_format: "b64_json" });
}
...
async function callTool(name, args) {
  ...
  if (name === "image_generate") {
    const result = await requestWithFallback(provider, args, (model) => providerRequest(provider, "/images/generations", { method: "POST", headers: { "Content-Type": "application/json" }, body: generationBody(args, model) }));
    return imageResult(result.image, args.output_path, result.fallbackUsed);
  }
  if (name === "image_edit") {
    const image = await getLocalImage(args.image_path, "image_path");
    const form = new FormData();
    const mask = args.mask_path ? await getLocalImage(args.mask_path, "mask_path") : null;
    const result = await requestWithFallback(provider, args, (model) => {
      const form = new FormData();
      form.append("model", model);
      form.append("prompt", args.prompt);
      form.append("image", new Blob([image.buffer], { type: image.mimeType }), image.filename);
      if (mask) form.append("mask", new Blob([mask.buffer], { type: mask.mimeType }), mask.filename);
      for (const field of ["size", "quality", "background"]) if (args[field]) form.append(field, args[field]);
      if (args.input_fidelity) form.append("input_fidelity", args.input_fidelity);
      form.append("response_format", "b64_json");
      return providerRequest(provider, "/images/edits", { method: "POST", body: form });
    });
    return imageResult(result.image, args.output_path, result.fallbackUsed);
  }
...
async function handle(message) {
  ...
  if (message.method === "initialize") return response(message.id, { protocolVersion: "2024-11-05", capabilities: { tools: {} }, serverInfo: { name: "ecommerce-image-studio", version: "1.0.0" } });
