...
- **Merge notes**: local text removal/replacement may merge with other same-output product-fidelity edits only when the target text is precise and the merged prompt can preserve all other text. For multiple difficult text regions, consider dense-layout (see SKILL.md Execution Mode Resolution).
...
Tool:
- `image_edit`
- Mode: **standard** for one local text region; **dense-layout** for multiple selected regions or difficult perspective/curved text. Resolve the `task_type` via SKILL.md **Execution Mode Resolution** (prefer `auto_generation`).

## Tool Invocation

- Tool: `image_edit`
- Mode: **standard** (default, single region) / **dense-layout** (multiple text regions or complex perspective/lighting) — resolve the `task_type` via SKILL.md **Execution Mode Resolution** (prefer `auto_generation`).
...
4. Multiple locations selected → treat as **dense-layout** (see SKILL.md Execution Mode Resolution)
...
- **Complex perspective/lighting**: when the original text has strong perspective, curved surface adherence, or 3D effects, treat as **dense-layout** (see SKILL.md Execution Mode Resolution)
