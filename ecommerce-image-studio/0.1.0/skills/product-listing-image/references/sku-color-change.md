...
- **Merge notes**: product recolor can merge with a background/scene change in one merged edit call (resolved mode) for a single output image. Generate one output per requested SKU variant.
...
> - "Change product color + change background" → multi-intent; load SKU Color Change + Scene Image, then use the Step 4 planner. Prefer one merged edit call (resolved mode) for a single output unless the edit requires dense layout or separate outputs.
