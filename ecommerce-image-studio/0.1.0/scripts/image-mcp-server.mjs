...
async function providerRequest(provider, path, options) {
  ...
  if (!request.ok) {
    ...
    throw new Error(`Image provider returned ${request.status}: ${detail || request.statusText}`);
  }
...
async function callTool(name, args) {
  ...
  if (name === "image_generate") return imageResult(await providerRequest(provider, "/images/generations", { method: "POST", headers: { "Content-Type": "application/json" }, body: generationBody(args, provider) }));
  if (name === "image_edit") {
    const image = await getLocalImage(args.image_path, "image_path");
    const form = new FormData();
    form.append("model", args.model ?? provider.model);
    ...
    return imageResult(await providerRequest(provider, "/images/edits", { method: "POST", body: form }));
  }
...
async function handle(message) {
  ...
  if (message.method === "initialize") return response(message.id, { protocolVersion: "2024-11-05", capabilities: { tools: {} }, serverInfo: { name: "ecommerce-image-studio", version: "0.1.0" } });
